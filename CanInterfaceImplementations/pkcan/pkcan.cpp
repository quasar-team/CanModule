...
/* virtual */ std::optional<CanModule::PORT_COUNTERS_t> PKCanScan::getHwCounters (){
	return {};
}
...
DWORD WINAPI PKCanScan::CanScanControlThread(LPVOID pCanScan)
{
	...
	while ( pkCanScanPointer->m_CanScanThreadRunEnableFlag ) {
		...
		if ( tpcanStatus == PCAN_ERROR_OK ) {
			...
			canMessage.c_eff = tpcanMessage.MSGTYPE & PCAN_MESSAGE_EXTENDED;
			canMessage.c_rtr = tpcanMessage.MSGTYPE & PCAN_MESSAGE_RTR;
