...
#define MLOGANA2(LEVEL,THIS) LOG( Log::LEVEL, AnaCanScan::st_logItHandleAnagate) << __FUNCTION__ << " " << " anagate bus= " << THIS->getBusName() << " "
 ...
 */
bool AnaCanScan::sendMessage(uint32_t cobID, unsigned char len, unsigned char *message, bool rtr, bool eff)
{
 ...
 */
bool AnaCanScan::sendRemoteRequest(uint32_t cobID)
{
	AnaInt32 anaCallReturn;
 ...
 */
OptionalVector<CanModule::PORT_LOG_ITEM_t> AnaCanScan::getHwLogMessages ( unsigned int n ){
	...
	if ( ret != ANA_ERR_NONE)
	{
		if(ret != m_diagnosticsReturnCodes.m_log)
		{
 ...
 */
std::optional<CanModule::HARDWARE_DIAG_t> AnaCanScan::getHwDiagnostics (){
	...
	if (ret != ANA_ERR_NONE || ret1 != ANA_ERR_NONE) {
		if(ret != m_diagnosticsReturnCodes.m_diagData || ret1 != m_diagnosticsReturnCodes.m_clientList)
		{
 ...
 */
std::string AnaCanScan::m_decodeNetworkByteOrder_ip_toString( unsigned int nip ){
	std::stringstream os;
 ...
 * and state
 */
std::optional<CanModule::PORT_COUNTERS_t> AnaCanScan::getHwCounters (){
	PORT_COUNTERS_t c;
	...
	if ( ret != ANA_ERR_NONE){
		if(ret != m_diagnosticsReturnCodes.m_counters)
		{
