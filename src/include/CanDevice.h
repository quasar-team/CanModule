...
struct CanDevice {
  ...
  int send(const CanFrame& frame) { return vendor_send(frame); };
  std::vector<int> send(const std::vector<CanFrame>& frames);

  std::string vendor_name() const { return m_vendor; }
 ...
 protected:
  CanDevice(std::string_view configuration) : m_configuration{configuration} {}
