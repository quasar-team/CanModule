...
def test_socketcan_single_message():
    ...
    myDevice1 = CanDevice.create(
        "socketcan", CanDeviceConfig("vcan0", received_frames_dev1.append)
    )
    myDevice2 = CanDevice.create(
        "socketcan", CanDeviceConfig("vcan0", received_frames_dev2.append)
    )
    myDevice1.open()
    myDevice2.open()
...
def test_socketcan_multiple_messages():
    ...
    assert len(received_frames_dev2) == 4
